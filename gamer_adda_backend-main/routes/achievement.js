...
router.post('/createexp', async (req, res) => {
    ...
    try {
        const user = await Achievement.create({
            userId: userId,
            pdfUrl: pdfUrl,
            summary: summary
        })
...
router.post('/expupdate', async (req, res) => {
    ...
    try {
        let update = await Achievement.findByIdAndUpdate({ _id:id }, { summary: summary, pdfUrl: pdfUrl });
        res.send(update)
    } catch (error) {
...
router.get('/getexp', async (req, res) => {
    try {
        ...
        let find = await Achievement.find({ userId: userid });
        res.send(find)
    } catch (error) {
        console.error(error.massage);
        res.status(500).send('interenal some Error are occuring.');
